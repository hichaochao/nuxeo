...
public class AvroServiceImpl
    ...
    public <D, M> D fromAvro(Schema schema, Class<D> clazz, M input) {
        return (D) ((AvroMapper<D, M>) getMapper(clazz)).fromAvro(schema, input);
    }
    ...
    public <D, M> M toAvro(Schema schema, D input) {
        return ((AvroMapper<D, M>) getMapper(input.getClass())).toAvro(schema, input);
    }
    ...
    protected AvroMapper<?, ?> getMapper(Class<?> clazz) {
