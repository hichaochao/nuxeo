...
public class DocumentModelMapper extends AvroMapper<DocumentModel, GenericRecord> {
    ...
    private static final String DOCUMENT_MODEL = "documentModel";
    ...
    public DocumentModel fromAvro(Schema schema, GenericRecord input) {
        ...
        GenericRecord documentTypeRecord = (GenericRecord) input.get("primaryType");
        ...
        for (Field schemaField : documentTypeRecord.getSchema().getFields()) {
            ...
            for (Field field : fields) {
                Object value = service.fromAvro(field.schema(), Property.class, schemaRecord.get(field.name()));
                data.put(service.decodeName(field.name()), value);
            }
    ...
    public GenericRecord toAvro(Schema schema, DocumentModel input) {
        ...
        if (DOCUMENT_MODEL.equals(logicalType)) {
            ...
            record.put(ID, input.getId());
        }
        for (Field field : schema.getFields()) {
            if (field.name().equals(ID) || field.name().equals(PATH)) {
                continue;
            }
